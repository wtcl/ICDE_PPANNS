...
void second_search(vector<vector<labeltype>> graph_search_result, vector<vector<int>> data_result, int finalK){
...
void ppanns_exp1(const char *database_path, string dataquery_path, string groundtruth_path){
    ...
    for(int efs = search_k; efs <= efs_max; efs += efs_step){
        ef_search = efs;
        auto graph_search_result = graph_search(dataquery, hnsw, search_k, data_result, 10);
    }
...
void ppanns_exp2(const char *database_path, string dataquery_path, string groundtruth_path){
    ...
    auto t1 = chrono::high_resolution_clock::now();
    ParallelFor(0,  points_num, 40, [&](size_t i, size_t threadId) {
        data_dce[i] = dce_enc(aspe_enc(data_vector.col(i)));
    });
    auto t2 = chrono::high_resolution_clock::now();
    ...
    auto t3 = chrono::high_resolution_clock::now();
    cout << "DCE data encrypt time = " << chrono::duration_cast<chrono::microseconds>(t2-t1).count() << endl;
    cout << "DCE query encrypt time = " << chrono::duration_cast<chrono::microseconds>(t3-t2).count() << endl;

    delete database_double;
    delete query_double;
    ...
    for(int k_num = search_k; k_num <= 128*search_k; k_num *= 2){
        for(int efs = k_num; efs <= efs_max; efs += efs_step){
            ef_search = efs;
            auto graph_search_result = graph_search(dataquery, hnsw, k_num, data_result, 10);
            second_search(graph_search_result, data_result, search_k);
...
void ppanns_exp3(const char *database_path, string dataquery_path, string groundtruth_path){
...
int main(int argc, char *argv[]){
    ...
    Eigen::initParallel();
    if (exp == 2){
        ppanns_exp2(database_path.data(), dataquery_path.data(), groundtruth_path.data());
    }else if (exp == 3){
        ppanns_exp3(database_path.data(), dataquery_path.data(), groundtruth_path.data());
    }else{
        ppanns_exp1(database_path.data(), dataquery_path.data(), groundtruth_path.data());
    }
